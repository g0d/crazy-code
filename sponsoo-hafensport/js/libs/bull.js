...
function bull()
{
    ...
    function utilities()
    {
        ...
        {
            if (!isNaN(val) && (val % 1 === 0) && (val.toString().indexOf('.') === -1))
                return true;
    ...
    function core()
    {
        ...
        function state_changed()
        {
            ...
            {
                if (utils.is_function(__timeout_callback))
                             ...
                             success_callback, request_timeout, timeout_callback, fail_callback)
        {
            if (utils.is_invalid(url) || (utils.is_invalid(data) && data !== '') || utils.is_invalid(element_id) || 
                ...
                (!utils.is_invalid(request_timeout) && (!utils.is_integer(request_timeout) || request_timeout < 1)) || 
                (!utils.is_invalid(timeout_callback) && (!utils.is_function(timeout_callback) || 
                 !utils.is_integer(request_timeout) || request_timeout < 1)))
                return false;
            
            __data_div_id = element_id;
            
            if (utils.is_bool(content_fill_mode))
                __content_fill_mode = content_fill_mode;
            
            if (utils.is_integer(request_timeout))
