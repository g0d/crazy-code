...
function lava()
{
    ...
    {
        if (!utils.is_object(definition_model))
            return false;
        ...
        {
            if (!utils.is_object(definition_model[def_counter]))
                return false;
            ...
            if (!utils.is_object(definition_model[def_counter].key) || 
                !utils.is_object(definition_model[def_counter].value))
                return false;
            ...
            if (!utils.is_string(definition_model[def_counter].key.id) || 
                !utils.is_bool(definition_model[def_counter].key.optional))
                return false;
            ...
            if (!utils.is_string(definition_model[def_counter].value.type) || 
            ...
            if (definition_model[def_counter].value.hasOwnProperty('length'))
            {
                ...
                if (!utils.is_integer(definition_model[def_counter].value.length) || 
            ...
            if (definition_model[def_counter].value.hasOwnProperty('regex'))
            {
                ...
                if (!utils.is_object(definition_model[def_counter].value.regex) || 
    ...
    this.validate = function()
    {
        ...
        for (def_counter = 0; def_counter < __json_def_model.length; def_counter++)
        {
            ...
            if (__json_def_model[def_counter].value.type !== '*')
            {
                ...
                {
                    if (!utils.is_array(this_field.value))
                        return false;
                }
                else
                {
